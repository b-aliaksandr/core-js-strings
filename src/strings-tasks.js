...
function formatTime(minutes, seconds) {
  ...
  const separator = ':';
  return formattedMinutes.concat(separator, formattedSeconds);
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < strLowerCase.length; i += 1) {
    if (vowels.includes(strLowerCase[i])) count += 1;
  }
...
function isPalindrome(str) {
  ...
  let preparedStr = str.toLowerCase();

  if ('!?.,'.includes(str.charAt(lastIndex))) {
    preparedStr = preparedStr.slice(0, lastIndex);
  }

  for (let i = 0, j = preparedStr.length - 1; i < preparedStr.length; ) {
    const startCodeCh = preparedStr[i].charCodeAt(0);
    const endCodeCh = preparedStr[j].charCodeAt(0);
    let isLetter = true;

    if (!(startCodeCh >= 97 && startCodeCh <= 122)) {
      i += 1;
      isLetter = false;
    }
    if (!(endCodeCh >= 97 && endCodeCh <= 122)) {
      j -= 1;
      isLetter = false;
    }
    if (isLetter) {
      if (startCodeCh !== endCodeCh) return false;
      i += 1;
      j -= 1;
    }
    isLetter = true;
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < sentence.length; i += 1) {
    const codeCh = sentence.charCodeAt(i);
    const isLetter =
      (codeCh >= 97 && codeCh <= 122) || (codeCh >= 65 && codeCh <= 90);

    if (!isLetter) {
      const curWordLen = i - curStartIndex;
      if (longestWord.len < curWordLen) {
        longestWord.startIndex = curStartIndex;
        longestWord.endIndex = i;
        longestWord.len = curWordLen;
      }
      curStartIndex = null;
    } else if (curStartIndex === null) {
      curStartIndex = i;
    }
  }

  return sentence.slice(longestWord.startIndex, longestWord.endIndex);
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const codeCh = str.charCodeAt(i);
    const isLowerLetter = codeCh >= 97 && codeCh <= 122;
    const isUpperLetter = codeCh >= 65 && codeCh <= 90;

    if (isLowerLetter) {
      sentence.push(str.charAt(i).toUpperCase());
    } else if (isUpperLetter) {
      sentence.push(str.charAt(i).toLowerCase());
    } else {
...
function encodeToRot13(str) {
  const AMOUNT_SHIFT = 13;
  const START_UPPER_INDEX = 65;
  const END_UPPER_INDEX = 90;
  const START_LOWER_INDEX = 97;
  const END_LOWER_INDEX = 122;

  const calcEncodedIndex = (curIndex, startLimit, endLimit) => {
    const resIndex = curIndex + AMOUNT_SHIFT;
    const amountFix = 1;

    return resIndex <= endLimit
      ? resIndex
      : startLimit + Math.abs(endLimit - curIndex - AMOUNT_SHIFT + amountFix);
  };

  const resArr = [...str].map((ch) => {
    const chIndex = ch.charCodeAt(0);

    if (chIndex >= START_UPPER_INDEX && chIndex <= END_UPPER_INDEX) {
      return calcEncodedIndex(chIndex, START_UPPER_INDEX, END_UPPER_INDEX);
    }
    if (chIndex >= START_LOWER_INDEX && chIndex <= END_LOWER_INDEX) {
      return calcEncodedIndex(chIndex, START_LOWER_INDEX, END_LOWER_INDEX);
    }
...
function getCardId(value) {
  const amountCardOfSuit = 13;
  const prioritySuit = { '♣': 0, '♦': 1, '♥': 2, '♠': 3 };
  const priorityCard = [
    'A',
    '2',
    '3',
  ...
  const [card, suit] = [value.slice(0, -1), value.at(-1)];

  return prioritySuit[suit] * amountCardOfSuit + priorityCard.indexOf(card);
}
